...
def _humanize_scope_items(items: tuple[str, ...]) -> str:
    """Render scope vocabulary identifiers as readable prose."""
...
A conforming verifier can use this pack to check: {_humanize_scope_items(SCOPE_PROVES_INTEGRITY)}, and any declared claim set.

Verification of this pack does **not** prove: {_humanize_scope_items(SCOPE_DOES_NOT_PROVE)}.

A PASS verdict means the evidence object is intact and the evaluated verdict channels passed. It does not imply the underlying work was safe, correct, or compliant. The per-run boundary of what was actually checked is recorded in the `scope` object of `verify_report.json`.
