        ...
        if self.a_analysis and self.b_analysis:
            ...
            d["summary"] = {
                "model_calls": {
                    "a": a.model_calls,
                    "b": b.model_calls,
                    "delta": b.model_calls - a.model_calls,
                },
                "total_tokens": {
                    "a": a.total_tokens,
                    "b": b.total_tokens,
                ...
                "cost_usd": {
                    "a": round(a.cost_usd, 4),
                    "b": round(b.cost_usd, 4),
            ...
            if a.latencies and b.latencies:
                d["latency_ms"] = {
                    "p50": {
                        "a": a.latency_p50,
                        "b": b.latency_p50,
                        "delta": (b.latency_p50 or 0) - (a.latency_p50 or 0),
                    },
                    "p95": {
                        "a": a.latency_p95,
                        "b": b.latency_p95,
                        "delta": (b.latency_p95 or 0) - (a.latency_p95 or 0),
                    },
                }
    ...
    if errors is not None:
        if result.b_analysis is not None:
            ...
            evaluation.results.append(
                GateResult(
                    name="errors",
                    threshold=float(errors),
                    actual=float(b_errors),
                    passed=b_errors <= errors,
                    unit="count",
                )
            )
        else:
...
def diff_packs(pack_a: Path, pack_b: Path, *, verify: bool = True) -> DiffResult:
    ...
    result.same_claim_set = (
        info_a.claim_set_hash == info_b.claim_set_hash and info_a.claim_set_hash != ""
    )
...
def explain_why(
    ...
    try:
        ...
        pass
    receipt_index = {r["receipt_id"]: r for r in receipts if r.get("receipt_id")}
