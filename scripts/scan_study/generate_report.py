...
def generate_report(rows: list[dict]) -> str:
    ...
    lines.append("")
    lines.append("```bash")
    lines.append("python3 -m pip install assay-ai")
    ...
    # How to challenge
    lines.append("## How to challenge this")
    lines.append("")
    lines.append("If you think your project has tamper-evident evidence emission and this scan missed it,")
    lines.append("[open an issue](https://github.com/Haserjian/assay/issues) with:")
    lines.append("")
    lines.append("- A commit link to the instrumentation code")
    lines.append("- The evidence emission pattern (signed receipts, hash chains, etc.)")
    lines.append("")
    lines.append("I'll verify it and update the dataset. The goal is accuracy, not a gotcha.")
    lines.append("")
    lines.append("If you think a finding is a false positive (e.g., `.invoke()` on a non-LLM object),")
    lines.append("same process -- open an issue with the file and line.")
    lines.append("")

    # Reproduce
    lines.append("## Reproduce this study")
    lines.append("")
    lines.append("```bash")
    lines.append("git clone https://github.com/Haserjian/assay.git")
    lines.append("cd assay")
    lines.append("python3 -m pip install -e .")
    lines.append("cd scripts/scan_study")
    lines.append("./run_study.sh                 # clone 30 repos, scan each, aggregate")
    lines.append("python generate_report.py      # generate report.md from results.csv")
    lines.append("```")
    lines.append("")
    lines.append("Artifacts:")
    lines.append("- `results/results.csv` -- per-repo metrics with commit SHAs")
    lines.append("- `results/summary.json` -- aggregate stats with tool version")
    lines.append("- `results/*.json` -- per-repo scan detail (30 files)")
    lines.append("- `results/report.md` -- this report")
    lines.append("")
