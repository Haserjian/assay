...
if ! "${PYTHON_BIN}" "${ROOT_DIR}/scripts/check_openclaw_release_slice.py"; then
  ...
  echo "Export a branch-backed isolated slice with python3 scripts/export_openclaw_release_slice.py --output <dir> --branch <review-branch> --run-smoke or remove out-of-scope changes before retrying package smoke." >&2
  exit 1
fi
