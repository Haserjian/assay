...
class TestInstructionInjection:
    ...
    def test_argument_embedded_injection_is_caught_by_deny_pattern(self):
        ...
        C. Claim boundary:
           CAN claim:  "argument-embedded injection was blocked"
           CANNOT claim: "result-embedded injection was blocked"

        Enforcement: CAUGHT (argument-side only)
        Evidence:    ATTRIBUTABLE

        When the attacker embeds the injection attempt in the ARGUMENTS
        (e.g., query contains override text), a deny_pattern can catch it.
        This is a different attack vector from result-embedded injection.
        """
