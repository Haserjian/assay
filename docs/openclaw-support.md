...
The deterministic demo is not live-fit validation. For the next milestone,
including how to generate one real exported session log in an isolated OpenClaw
profile and rerun the Assay harness against it, see
[OpenClaw Live Validation Runbook](openclaw-live-validation-runbook.md).
