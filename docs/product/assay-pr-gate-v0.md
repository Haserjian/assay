...
- optional `claim_gate_report` generated by `assay claim-gate diff`
- policy profile
- risk path matches
...
The Claim channel is driven by embedded `claim_gate_report` evidence when that
report is present in `pr_gate_evidence.json`:
...
`NEEDS_REVIEW` maps to channel `FAIL` in v0.1 because the channel has no
...
If Assay itself runs a command, write only:
...
> Tests cover risk path, missing check, failed check, untrusted signer,
> claim_gate report mapping, and clean PR.
